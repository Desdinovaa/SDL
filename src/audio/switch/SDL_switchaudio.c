...
SWITCHAUDIO_OpenDevice(SDL_AudioDevice *device)
{
    ...
    const SDL_AudioFormat *test_format;
    Result res;
    u32 size;
    int mpid;
    ...
    test_format = SDL_ClosestAudioFormats(device->spec.format);
    ...
    device->hidden->pool = SDL_aligned_alloc(0x1000, size);
    for (int i = 0; i < 2; i++) {
        ...
        device->hidden->buffer_tmp = malloc(device->buffer_size);
    }
...
SWITCHAUDIO_PlayDevice(SDL_AudioDevice *device, const Uint8 *buffer, int buflen)
{
    ...
    for (int i = 0; i < 2; i++) {
        if (device->hidden->buffer[i].state == AudioDriverWaveBufState_Free
            || device->hidden->buffer[i].state == AudioDriverWaveBufState_Done) {
            current = i;
            break;
        }
    }
...
static bool
SWITCHAUDIO_WaitDevice(SDL_AudioDevice *device)
{
    return true;
}

static Uint8
*SWITCHAUDIO_GetDeviceBuf(SDL_AudioDevice *device, int *buffer_size)
{
...
static void
SWITCHAUDIO_ThreadInit(SDL_AudioDevice *device)
{
