...
static void SWITCH_UpdateControllerSupport(bool handheld) {
    ...
    for (int i = 0; i < JOYSTICK_COUNT; i++) {
        SDL_Joystick *joy = SDL_GetJoystickFromID(i);
        if (joy) {
 ...
 * It should return true, or false on an unrecoverable fatal error.
 */
...
/* Function to open a joystick for use.
   ...
   It returns true, or false if there is an error.
 */
 ...
 * but instead should call SDL_SendJoystick*() to deliver events
 * and update joystick device state.
 */
static void SWITCH_JoystickUpdate(SDL_Joystick *joystick) {
    u64 diff;
    int index = (int) SDL_GetJoystickID(joystick);
